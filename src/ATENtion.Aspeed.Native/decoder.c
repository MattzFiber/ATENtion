...
#if defined(_WIN32)
#define ASPEED_EXPORT __declspec(dllexport)
#else
#define ASPEED_EXPORT __attribute__((visibility("default")))
#endif
...
static void set_quant_table(BYTE* basic_table, BYTE scale_factor, int32_t* newtable)
{
    BYTE i;
    int32_t temp;

    for (i = 0; i < 64; i++) {
        temp = ((int32_t)(basic_table[i] * 16) / scale_factor);
...
static void load_quant_table(int32_t* quant_table)
{
    ...
    for (row = 0; row < 8; row++)
        for (col = 0; col < 8; col++) {
            quant_table[j] = (int32_t)((quant_table[j] * scalefactor[row] * scalefactor[col]) * 65536);
            j++;
        }
}

static void load_quant_tableCb(int32_t* quant_table)
{
    float scalefactor[8] = { 1.0f, 1.387039845f, 1.306562965f, 1.175875602f,
                             1.0f, 0.785694958f, 0.541196100f, 0.275899379f };
    BYTE j = 0, row, col;
...
static void load_advance_quant_table(int32_t* quant_table)
{
    float scalefactor[8] = { 1.0f, 1.387039845f, 1.306562965f, 1.175875602f,
                             1.0f, 0.785694958f, 0.541196100f, 0.275899379f };
    BYTE j = 0, row, col;
...
static void load_advance_quant_tableCb(int32_t* quant_table)
{
    float scalefactor[8] = { 1.0f, 1.387039845f, 1.306562965f, 1.175875602f,
                             1.0f, 0.785694958f, 0.541196100f, 0.275899379f };
    BYTE j = 0, row, col;
...
{
    if (newbits <= bits) {
        uint32_t data = in_buf[in_buf_index++];
